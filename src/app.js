...
const origins=(process.env.FRONTEND_URL||"http://localhost:3000,http://127.0.0.1:3000,http://localhost:3001,http://127.0.0.1:3001,http://localhost:5173").split(",").map(x=>x.trim()).filter(Boolean);
...
app.use(rateLimit({windowMs:Number(process.env.RATE_LIMIT_WINDOW_MS||60000),max:Number(process.env.RATE_LIMIT_MAX||600),standardHeaders:"draft-8",legacyHeaders:false,skip:(req)=>req.path.startsWith("/api/leaderboard/stream")||(req.method==="GET"&&limiterSafeReads.has(req.path))||req.path.startsWith("/api/admin"),message:{message:"Juda ko‘p so‘rov. Bir ozdan keyin yana urinib ko‘ring."}}));
...
app.use("/api",auth,profile,users,leaderboard,content,dashboard,challenges,projects,admin,competitions);
